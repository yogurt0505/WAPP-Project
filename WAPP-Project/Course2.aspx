<%@ Page Title="" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="Course2.aspx.cs" Inherits="WAPP_Project.Course2" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    ...
    <ol>
        <li>Remove the 다  먹</li>
        <li>The last vowel is ㅓ, which means we add 어  먹어</li>
    ...
    <ol>
        ...
        <li>The last vowel is ㅗ, which means we add 아  좋아</li>
    ...
    <ol>
        <li>Remove the 다  오<br />오 ends in a vowel (it has no consonants at the bottom)</li>
        <li>The last vowel is ㅗ, so we add 아  오아<br />
            But because 오 ends in a vowel (Step 1), we simply combine the 아  와<br />
            (Here, you can see clearly that it’s actually combining)</li>
    ...
    <ol>
        ...
        <li>The last vowel is ㅜ, so we add 어  주어<br />
            But because 주 ends in a vowel (Step 1), we simply combine the 어  줘<br />
            (Here it is also easy to see that it’s combining together)</li>
    ...
    <ol>
        <li>Remove the 다  마시<br />
            (시 ends in ㅣ)</li>
        <li>The last vowel is ㅣ, so we add 어  마시어<br />
            Because 마시 ends in ㅣ (Step 1), we combine the 어  마셔</li>
    ...
    <ol>
        <li>Remove the 다  가르치<br />
            (치 ends in ㅣ)</li>
        <li>The last vowel is ㅣ, so we add 어  가르치어<br />
            Because 가르치 ends in ㅣ (Step 1), we combine the 어  가르쳐</li>
    ...
    <ol>
        <li>Remove the 다  바쁘<br />(쁘 ends in ㅡ)<br />
            Because the verb stem ends in ㅡ, look back one more space (바)</li>
        <li>The 2<sup>nd</sup> to last syllable (바) ends in ㅏ, so we will be adding 아  바쁘 + 아<br />
            But first, remove the ㅡ  바ㅃ<br />
    ...
    <ol>
        <li>Remove the 다  예쁘<br />
            (쁘 ends in ㅡ)<br />
            Because the verb stem ends in ㅡ, look back one more space (예)</li>
        <li>The 2<sup>nd</sup> to last syllable (예) doesn’t end in ㅗ or ㅏ, so we’ll add 어  예쁘 + 어<br />
            But first, remove the ㅡ  예ㅃ<br />
            Then add the 어  예뻐</li>
    ...
    <p><b>“But what if the verb only has 2 syllables? There is no ‘2<sup>nd</sup> to last syllable’ to look at!”</b><br />
        ...
        any extra steps to find it. Simply remove the ㅡ as before, 
    ...
    <ol>
        <li>Remove the 다  쓰<br />
            (쓰 ends in ㅡ)<br />
            Normally you’d look back one more space, but this is all there is.</li>
        <li>The last vowel is ㅡ (which isn’t ㅗ or ㅏ), so we always add 어  쓰 + 어<br />
            But first, remove the ㅡ  ㅆ<br />
            Then add the 어  써</li>
    ...
    order to conjugate it. After removing the ㅡ, if you see a ㄹ left alone, the ㄹ makes a copy
    of itself and moves to the bottom of the 2<sup>nd</sup>
    to last syllable. It sounds strange, but don’t
    ...
    <ol>
        <li>Remove the 다  모르<br />
            (모르 ends in 르, and 르 also ends in ㅡ, so many of the rules are similar)<br />
            Because the verb stem ends in ㅡ, look back one more space (모)</li>
        <li>The 2<sup>nd</sup> to last syllable (모) ends in ㅗ, so we will be adding 아  모르 + 아<br />
            But first, remove the ㅡ  모ㄹ<br />
            There is a ㄹ by itself, so make a friend for it to play with at the bottom of the 2<sup>nd</sup> to<br />
            last syllable  몰ㄹ<br />
    ...
    <ol>
        <li>Remove the 다  부르<br />
            (부르 ends in 르, and 르 also ends in ㅡ, so many of the rules are similar again)<br />
            Because the verb stem ends in ㅡ, look back one more space (부)</li>
        <li>The 2<sup>nd</sup> to last syllable (부) ends in ㅜ (which isn’t ㅗ or ㅏ), so we will be adding 어<br />
            before we’re finished  부르 + 어<br />
            But first, remove the ㅡ  부ㄹ<br />
            There is a ㄹ by itself, so make a copy and put it at the bottom of the 2<sup>nd</sup> to last<br />
            syllable  불ㄹ<br />
            Then add the 어  불러</li>
    ...
    there’s an extra step. Remove the ㅂ, and add a 우; then add 어 (since 우 does not end in ㅗ
    ...
    <ol>
        <li>Remove the 다  부럽<br />
            (부럽 ends in ㅂ)</li>
        <li>Since there’s a ㅂ here, we will be trading it away for a 우, and 우 is not ㅗ or ㅏ so<br />
            we will be conjugating this by adding 어  부럽 + 어<br />
            Remove the ㅂ  부러<br />
            Add a 우  부러우<br />
            Now add on the 어  부러우어<br />
            <b>But wait!</b> 부러우 ends in a vowel with no consonants on the bottom, so we should<br />
            combine the ending here  부러워</li>
    ...
    <ol>
        <li>Remove the 다  맵<br />
            (맵 ends in ㅂ)</li>
        <li>Since there’s a ㅂ here, we will be trading it away for a 우, and 우 is not ㅗ or ㅏ so<br />
            we will be conjugating this by adding 어  맵 + 어<br />
            Remove the ㅂ  매<br />
            Add a 우  매우<br />
            Now add on the 어  매우어<br />
            매우 ends in a vowel with no consonants on the bottom, so we should combine the<br />
            ending here too  매워요</li>
    ...
    <ol>
        <li>Remove the 다  씹</li>
        <li>The last vowel is ㅣ (not ㅗ or ㅏ), which means we add 어  씹어</li>
    ...
    <ol>
        <li>Remove the 다  하</li>
        <li>This is a special verb, so it does not add 아 – it just becomes 해*</li>
    ...
    becomes “이에요” when used after a consonant, or “예요” when used after a vowel. However,
    you cannot simply remove the 요 here to make it casual; if you want to say it without the 요
    ...
    <p style="text-decoration: underline;">Casual:</p>
    <ul>
        <li>Consonant + 이야</li>
        <li>Vowel + 야</li>
    </ul>
